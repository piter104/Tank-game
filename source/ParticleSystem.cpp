...
void ParticleSystem::processSystem(int n, float timestep) {
	if (count > 0) {
		...
		for (int i = 0; i < n; i++)
		{
			system[i].position += system[i].speed * timestep; //przesuniêcie
			...
			system[i].speed *= gravity; //uwzglêdnienie grawitacji

			system[i].ttl -= timestep; //Skrócenie czasu ¿ycia cz¹stki
			if (system[i].ttl > 0) {
...
void ParticleSystem::drawParticles(glm::mat4 P, glm::mat4 V, ShaderProgram* sp, glm::mat4 M_lufa)
{
	...
	glVertexAttribPointer(sp->a("vertex"), 4, GL_FLOAT, false, 0, v); //Wspó³rzêdne wierzcho³ków bierz z tablicy birdVertices

	glEnableVertexAttribArray(sp->a("color"));
	glVertexAttribPointer(sp->a("color"), 4, GL_FLOAT, false, 0, colors_particles); //Wspó³rzêdne wierzcho³ków bierz z tablicy birdColors
